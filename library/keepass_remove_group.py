...
from ansible.module_utils.keepass_func import check_pykeepass_present, check_variables, open_database, attempt_lock, get_group, remove_group
import datetime
...
DOCUMENTATION = r'''
---
module: keepass_remove_group

short_description: This module removes group in KeePass database.

# If this is part of a collection, you need to use semantic versioning,
# i.e. the version is of the form "2.5.0" and not "2.4".
version_added: "1.0.0"

description: This module removes group in KeePass database.
...
options:
    ...
    group:
        description:
            - Path to the group which will be removed. Each group must by separated by '/'. Example: test-group/subgroup/subsubgroup.
        required: true
        type: str
...
database:
    ...
    type: str
group:
    description: Path to the group which was removed.
    type: str
msg:
...
def run_module():
    ...
    with attempt_lock(module, database, timeout):
        ...
        if module.check_mode:
            if not get_group(kp, group, True):
                module.exit_json(changed=False, msg=f"No group '{group}' in database.")

        if not module.check_mode:
            try:
                removal = remove_group(kp, group)
                if not removal:
                    module.exit_json(changed=False, msg=f"No group '{group}' in database.")
        ...
        result['group'] = group
        result['msg'] = "Group successfuly removed."
